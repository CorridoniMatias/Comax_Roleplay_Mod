...
//Comax Defines:
#define RPVERSION "4.2.1b0011"
#define DEBUG 1 //THIS SHOULD REMAIN TO 0 UNLESS YOU'RE TESTING!
...
public Action:Command_ModAbout(client, args) 
{
	...
	PrintToChat(client, "|Copyright 2011 - 2012 The MatCor Team. All rights reserved.|");
	PrintToChat(client, "|=================== Comax RolePlay Mod ====================|");
	PrintToChat(client, "|===========================================================|");
	new String:Message[600];
	
	Message = "Comax RolePlay Mod by Reloaded\nNew Features: PickPocketing Mod, Money Printers, Clock, Custom Models,Clients' stats, Cars System.\nCopyright 2011 - 2012 The MatCor Team. All rights reserved.";
...
public OnClientPutInServer(Client)
{
	...
	//Register:
	PrintToChat(Client, "\x01[RP] Comax RolePlay Mod v%s by \x04[CMX] Reloaded\x01 + Roleplay_Remixed by EasSidez (Original mod by \x04[GR]Nick_6893{A}\x01) loaded successfully!", RPVERSION);
	saveUserName(Client);
}
...
public Save(Client)
{
	...
	if(Loaded[Client] && Minutes[Client] >= 1)
	{
		...
		if(Client != 0)
		{
			...
			{
				Neg[Client] = 0;
				SaveInteger(SaveVault, "Bank", SteamId, Bank[Client]);
			}
			if(Bank[Client] < 0)
			{
				Neg[Client] = 0 - Bank[Client];
				SaveInteger(SaveVault, "Bank", SteamId, 0);
			}
			
			SaveInteger(SaveVault, "Money", SteamId, Money[Client]);
			SaveInteger(SaveVault, "Wages", SteamId, Wages[Client]);
			SaveInteger(SaveVault, "Minutes", SteamId, Minutes[Client]);
			SaveInteger(SaveVault, "Crime", SteamId, Crime[Client]);
			SaveInteger(SaveVault, "Exploit", SteamId, ExploitJail[Client]);
			SaveInteger(SaveVault, "Pruned", SteamId, Prune[Client]);
			SaveInteger(SaveVault, "Negative", SteamId, Neg[Client]);
			SaveInteger(SaveVault, "Checks", SteamId, NumChecks[Client]);
			SaveInteger(SaveVault, "ExpRebel", SteamId, ExpRebel[Client]);
			SaveInteger(SaveVault, "ExpCombine", SteamId, ExpCombine[Client]);
			SaveInteger(SaveVault, "TimeJail", SteamId, TimeConverter[Client]);
			SaveInteger(SaveVault, "Grams", SteamId, Grams[Client]);
			//SaveInteger(SaveVault, "Props", SteamId, PropLimit[Client]);
			SaveInteger(SaveVault, "MainHudColor", SteamId, MainHudColor[Client]);
			SaveInteger(SaveVault, "CenterHudColor", SteamId, CenterHudColor[Client]);
			SaveInteger(SaveVault, "ExpLevel", SteamId, ExpLevel[Client]);
			SaveInteger(SaveVault, "Planted", SteamId, Planted[Client]);
			SaveInteger(SaveVault, "CuffCount", SteamId, CuffCount[Client]);
			...
			for(new X = 0; X < MAXITEMS; X++) 
			{
				...
				//Save:
				SaveInteger(SaveVault, ItemId, SteamId, Item[Client][X]);
			}
...
public Load(Client)
{
	...
	//Variables (Integer):
	Money[Client] = LoadInteger(SaveVault, "Money", SteamId, DEFAULTMONEY);
	Bank[Client] = LoadInteger(SaveVault, "Bank", SteamId, DEFAULTBANK);
	Wages[Client] = LoadInteger(SaveVault, "Wages", SteamId, DEFAULTWAGES);
	Minutes[Client] = LoadInteger(SaveVault, "Minutes", SteamId, 0);
	Crime[Client] = LoadInteger(SaveVault, "Crime", SteamId, 0);
	ExploitJail[Client] = LoadInteger(SaveVault, "Exploit", SteamId, 0);
	Prune[Client] = LoadInteger(SaveVault, "Pruned", SteamId, 0);
	Neg[Client] = LoadInteger(SaveVault, "Negative", SteamId, 0);
	NumChecks[Client] = LoadInteger(SaveVault, "Checks", SteamId, 10);
	Neg[Client] = LoadInteger(SaveVault, "Negative", SteamId, 0);
	ExpRebel[Client] = LoadInteger(SaveVault, "ExpRebel", SteamId, 0);
	ExpCombine[Client] = LoadInteger(SaveVault, "ExpCombine", SteamId, 0);
	TimeConverter[Client] = LoadInteger(SaveVault, "TimeJail", SteamId, 0);
	Grams[Client] = LoadInteger(SaveVault, "Grams", SteamId, 0);
	//PropLimit[Client] = LoadInteger(SaveVault, "Props", SteamId, 0);
	MainHudColor[Client] = LoadInteger(SaveVault, "MainHudColor", SteamId, DEFAULTMAINHUDCOLOR);
	CenterHudColor[Client] = LoadInteger(SaveVault, "CenterHudColor", SteamId, DEFAULTCENTERHUDCOLOR);
	Planted[Client] = LoadInteger(SaveVault, "Planted", SteamId, 0);
	ExpLevel[Client] = LoadInteger(SaveVault, "ExpLevel", SteamId, 0);
	CuffCount[Client] = LoadInteger(SaveVault, "CuffCount", SteamId, 0);
...
public setDoorRights(Client,var)
{
	decl ClientID;
	//Initialize:
	ClientID = GetClientUserId(Client);
	
	ServerCommand("sm_copdoor #%d %d", ClientID, var);
}

public setDoorRights2(Client,var)
{
	decl ClientID;
	//Initialize:
	ClientID = GetClientUserId(Client);
	
	ServerCommand("sm_firedoor #%d %d", ClientID, var);
}
...
public Plugin:myinfo =
{
	...
	//Initialize:
	name = "Comax Roleplay Mod + Roleplay_Remixed",
	author = "Reloaded(Comax RP Mod) & EasSide[-ZZ-] (Roleplay_Remixed)",
...
public OnMapStart()
{
	/************** Comax Mod **************/
	ComaxMapStart();
	//LOADVIPDOORS GETS CALLED AT THE END OF THIS FORWARD!!!!
...
public Action:weteest(client, args)
{
	testdisnig();
	PrintToServer("BREAK.-----------------------");
	testdisnig2();
...
public OnPluginStart()
{
	...
	if(StrEqual(FIP, "209.222.14.34:27015")) //StrEqual(g_ServerIp, "192.168.0.101") || 209.222.14.34
	{
...
public Action:CommandRemGambling(Client, Args)
{
	...
	decl Refresh;
	Refresh = LoadInteger(SaveVault, "Bank", SteamId, DEFAULTBANK);
	Refresh += Balance;
	SaveInteger(SaveVault, "Bank", SteamId, Refresh);
	return Plugin_Handled;
}
