...
#pragma dynamic 131072 //Fixes a weird bug with large arrays. SourceMod is COMPLETLY gay.
...
{
	name = "Entity Locker",
	author = "Krim - Edited by [CMX] Reloaded - Previously edited by [GR]Nick.",
	description = "Saves Entitys",
	version = "2.0",
	url = "www.comaxservers.com.ar"
};
...
public OnPluginStart()
{
	...
	PrintToServer("RP: Initializing Nicks Entity Restorer. Mod by Reloaded.");
	//CreateConVar("rp_version", "1.3", "Nicks Entity Restorer Version",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_UNLOGGED|FCVAR_DONTRECORD|FCVAR_REPLICATED|FCVAR_NOTIFY);
	Command_lastID();
}
...
public Action:Command_Saveit(Client,args)
{
	...
	if(Ent != -1 && Ent > GetMaxClients())
	{
		...
		{
			PrintToChat(Client,"[RP]: Model doesnt seem to be correct: %s",modelname);
			return Plugin_Handled; 
		}
		...
		if(StrEqual(Props, "Null", false))
		{
			...
			Entys[Ent] = lastID;
			PrintToChat(Client,"[RP]: Save #%i: Properties: %s",lastID,SaveBuffer);
			...
			{
				PrintToConsole(owner, "[Comax] An admin saved your prop!");
				
				ServerCommand("comax_removeentity %d %d", owner, Ent);
				
				DispatchKeyValue(Ent, "targetname", "-1"); //Reset owner
			} else 
				PrintToServer("ERROR: cannot remove entity from client. Invalid client. Owner reported: %d", owner);
			//End - Comax
			return Plugin_Handled;
		}
		//PrintToChat(Client, "[RP] Detected Already Used ID. Finding another open ID.");
		
		CloseHandle(Vault);
		WriteID(Client, SaveBuffer, Ent);
		return Plugin_Handled;
	}
	PrintToChat(Client, "[RP] Error. Invalid Entity.");
	return Plugin_Handled;
}
...
stock WriteID(Client, String:SaveBuffer2[255], Ent)
{
	...
	if(StrEqual(Props, "Null", false))
	{
		...
		Entys[Ent] = lastID;
		PrintToChat(Client, "[RP]: Save #%i: Properties: %s",lastID,SaveBuffer2);
	}
	else
	{
		CloseHandle(Vault);
...
public bool:Command_lastID()
{
	...
	PrintToServer("RP: new lastID: #%d",lastID);
...
{
	PrintToServer("RP: Loading Entities...");
	decl Handle:Vault;
	decl String:Props[255];
	...
	PrintToServer("RP: Found %d Entities",Y-1);
	KvRewind(Vault);
	...
	for(new X = 0; X < Y; X++)
	{
		if(loadArray[X] == 0)
		{
			PrintToServer("RP: Error at Entity #%d",X);
			CloseHandle(Vault);
			return Plugin_Handled;
		}
		...
		if(StrContains(Props, "Null", false) == -1)
		{
			...
			{
				PrintToServer("RP: Entry %d can not be a valid model",loadArray[X]);
			} 
		}
	}
	PrintToServer("RP: All Entities loaded");
	CloseHandle(Vault);
	return Plugin_Handled;
}
...
public Action:CommandRemoveEnt(Client, Args)
{
	...
	if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
	{
		if(Entys[Ent])
		{
			...
			//Print:
			if(!Deleted) PrintToConsole(Client, "[RP] Failed to remove Entity %d from the database", Entys[Ent]);
			else 
			{
				//Print:
				PrintToConsole(Client, "[RP] Removed Entity %d from the database", Entys[Ent]);
				Entys[Ent] = 0;
				RemoveEdict(Ent);
			}
...
public Action:Command_Skin(Client,args)
{
	...
	if(args < 1)
	{
		PrintToConsole(Client, "[RP] Usage: sm_changeskin <skin number> <opt 0|1 save>");
		return Plugin_Handled;
	}
	else if(args == 1)
	{
		...
		if(Var < 0)
		{
			PrintToChat(Client, "[RP] Usage: sm_changeskin <skin number> <opt 0|1 save>");
			return Plugin_Handled;
		}
		decl Ent;
		Ent = GetClientAimTarget(Client, false);
		...
		if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
		{
			...
			PrintToChat(Client, "[RP] Changed Skin temporarily to #%s", Arg1);
			return Plugin_Handled;
		}
		PrintToChat(Client, "[RP] Could not locate a prop.");
		return Plugin_Handled;
	}
	else if(args > 1)
	{
		...
		if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
		{
			...
			if(Var2 > 0)
			{
				if(Entys[Ent] < 1)
				{
					PrintToChat(Client, "[RP] This prop must be saved first before you save its skin");
					return Plugin_Handled;
				}
				DispatchKeyValue(Ent, "skin", Arg1);
				PrintToChat(Client, "[RP] Changed Skin to #%s and is saved", Arg1);
				decl Handle:Vault;
				Vault = CreateKeyValues("Vault");
				FileToKeyValues(Vault, NPCPath);
...
public Action:Command_Color(Client,args)
{
	...
	if(args < 4)
	{
		PrintToConsole(Client, "[RP] Usage: sm_changecolor <Red 0-255> <Green 0-255> <Blue 0-255> <Alpha 0-255> <opt 0|1 save>");
		return Plugin_Handled;
	}
	else if(args == 4)
	{
		...
		{
			PrintToChat(Client, "[RP] Usage: sm_changecolor <Red 0-255> <Green 0-255> <Blue 0-255> <Alpha 0-255> <opt 0|1 save>");
			return Plugin_Handled;
		}
		decl Ent;
		Ent = GetClientAimTarget(Client, false);
		...
		if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
		{
			...
			PrintToChat(Client, "[RP] Changed Color temporarily to Red=%d Green=%d Blue=%d Alpha=%d", Var, Var2, Var3, Var4);
			return Plugin_Handled;
		}
		PrintToChat(Client, "[RP] Could not locate a prop.");
		return Plugin_Handled;
	...
	else if(args > 4)
	{
		...
		if(Ent != -1 && Ent > 0 && Ent > GetMaxClients())
		{
			...
			if(Var5 > 0)
			{
				if(Entys[Ent] < 1)
				{
					PrintToChat(Client, "[RP] This prop must be saved first before you save its color/alpha");
					return Plugin_Handled;
				}
				SetEntityRenderMode(Ent, RENDER_GLOW);
				SetEntityRenderColor(Ent, Var, Var2, Var3, Var4);
				PrintToChat(Client, "[RP] Changed Color to Red=%d Green=%d Blue=%d Alpha=%d and is saved.", Var, Var2, Var3, Var4);
				decl Handle:Vault;
				Vault = CreateKeyValues("Vault");
				FileToKeyValues(Vault, NPCPath);
