...
public OnPluginStart()
{
	...
	RegServerCmd("vehiclemod_spawnatcoordscomax", SpawnVehicleCoordsReloaded, "Spawns a vehicle at a set of coordinates. Usage vehiclemod_spawnatcoords <x coord> <y coord> <z coord> <yaw angle> <vehicle name> client)");
	RegServerCmd("ForceClientExitCarEqualsOne", ForceCarExit, "force client exit car");
	RegServerCmd("setcanturnonforcarcomaxroleplay", commandsetturnon, "force client exit car");
	...
	PrintToServer("VehicleMod: Running!");
...
public Action:commandsetturnon(args)
{
	if(args < 2) return Plugin_Handled;
	
	new String:arg1[5], String:arg2[5];
	
	GetCmdArg(1, arg1, 5); //VehicleID
	GetCmdArg(2, arg2, 5); //Value.
	
	CanTurnOn[StringToInt(arg1)] = StringToInt(arg2);
...
public Action:SpawnVehicleCoordsReloaded(args)
{
	if ((GetCmdArgs() != 6) && (GetCmdArgs() != 5))
	{
		PrintToServer("********** vehiclemod: must be 5 or 6 arguments - <x coord> <y coord> <z coord> <yaw angle> <vehicle name> client");
		return Plugin_Handled;
	}
	...
	SpawnVehicle(SpawnOrigin, SpawnAngles, Arg5, 0, client);
...
{	
	...
	if (VehicleIndex == -1)
	{
		...
		PrintToChat(client, "[Comax]: Error at creating ent. Please contact an admin.");
		ClientCommand(client, "sm_destroycar restore");
		return;
	}
	...
	PrintToServer("Creating car with EntIndex %i", VehicleIndex);
	DispatchKeyValue(VehicleIndex, "targetname", TargetName);
	DispatchKeyValue(VehicleIndex, "classname", TargetName);
	
	if(client > 0)
	{ 	//Send Data to the RP Mod
		ServerCommand("sm_setcarindex %i %d", VehicleIndex, client);
	}
	...
	if (PhysIndex == -1)
	{
		...
		PrintToChat(client, "[Comax]: Error at creating flip. Please contact an admin.");
		ClientCommand(client, "sm_destroycar restore");
		return;
	}
	...
	if (GunIndex == -1)
	{
		...
		PrintToChat(client, "[Comax]: Error at creating G.U.N. Please contact an admin.");
		ClientCommand(client, "sm_destroycar restore");
		return;
	}
	...
	if (DamageIndex == -1)
	{
		...
		PrintToChat(client, "[Comax]: Error at creating DMG. Please contact an admin.");
		ClientCommand(client, "sm_destroycar restore");
		return;
	}
	...
	if (TR_DidHit())
	{
		...
		PrintToChat(client, "[Comax]: Error at getting your position. Please contact an admin.");
		ClientCommand(client, "sm_destroycar restore");
		return;
	}
	*/
...
public Action:ForceCarExit(args)
{
	new String:arg1[5], String:arg2[5];
	...
	new vint = StringToInt(arg1);
	new cint = StringToInt(arg2);
	
	ExitVehicle(cint, vint);
	
	PrintToServer("Forcing client %d out of car id %i", cint, vint);
	return Plugin_Handled;
}

ExitVehicle(client, vehicle, bool:force = false)
{
	...
	ServerCommand("comax_client_exit_car %d", client); //Connection with Comax Mod.
}
...
public Action: OnClientTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	if (damagetype & DMG_VEHICLE)
	{
		...
		if (StrEqual("prop_vehicle_driveable", ClassName, false))
		{
			...
			if (Driver != -1)
			{
				PrintToChat(Driver, "[Comax] Please DO NOT run over players.");
				return Plugin_Handled;
				/*
				damage *= 2.0;
